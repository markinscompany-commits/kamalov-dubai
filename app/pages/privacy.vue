<!--
  ...
  ⚠️ Формулировки специально подобраны так, чтобы остаться ПРАВДОЙ после
  запуска: раньше здесь стояло «данные не передаются третьим лицам» и
  «не используются для рекламы» - обе фразы стали бы ложью в момент
  подключения доставки заявок и рекламного пикселя, а расхождение политики
  с фактом Meta трактует как введение в заблуждение. Теперь названы и
  сервисы доставки, и передача за пределы ОАЭ, и статистика с куками.
...
useHead(() => ({
  ...
  // В заголовке вкладки - «Dr Kamalov» по норме английского (строчное «dr.»
  // на странице оставлено намеренно: это начертание логотипа)
