...
.header {
  ...
  z-index: 100;
  /*
    ⚠️ Здесь БЫЛ transform: translateZ(0) — ручное вынесение шапки в отдельный слой.
    Убран сознательно, по двум причинам:
    · любой transform делает элемент точкой отсчёта для закреплённых потомков.
      Меню разделов из-за этого схлопывалось до высоты шапки;
    · закреплённые элементы браузеры и так держат в отдельном слое, а на телефоне
      принудительный слой во время сворачивания адресной строки скорее отстаёт,
      чем помогает.
    Не возвращать без замера на живом устройстве.
  */
}
...
/* --- Телефон ---
   ...
   Здесь шапка работает иначе, чем на десктопе.

   Было: одна и та же закреплённая шапка плавно перекрашивалась из светлой в тёмную.
   Логотип при этом 400 мс висел серым поверх наполовину проявившейся подложки, и
   в этот момент он то читался, то нет — Марк отметил это как «смена логотипа
   происходит странно».

   Стало: два состояния не смешиваются. Верхний вид пропадает мгновенно, а полоса
   после прокрутки ВЫЕЗЖАЕТ сверху как отдельный элемент. Никаких промежуточных
   полупрозрачных состояний, в которых логотип нечитаем. */
...
@media (max-width: 900px) {
  ...
  /* Высоту не анимируем: изменение размера каждый кадр пересчитывает раскладку.
     Цвет и подложку тоже — они переключаются вместе с выездом, а не плавно */
  .header__inner,
  .header--scrolled .header__inner {
    block-size: var(--header-h);
    transition: none;
  ...
  /* Полоса выезжает сверху ровно один раз — в момент, когда состояние переключилось.
     backwards, а не both: сдвиг живёт только пока идёт выезд. После него у шапки
     не остаётся никакого transform — на iOS закреплённый элемент с ним дёргается */
  .header--scrolled {
    animation: bar-in 320ms var(--ease-out) backwards;
  }
}
...
@media (max-width: 720px) {
  ...
  /* WhatsApp живёт в меню — в шапке телефона хватает записи и бургера.
     Переключатель языка остаётся: вверху страницы он виден сразу, после прокрутки
     гаснет и уступает место кнопке записи (они делят одну ячейку) */
