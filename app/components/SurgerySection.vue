...
/*
  Проход, который сейчас не читают, приглушён - но остаётся читаемым: прятать
  текст нельзя, это медицинские сведения.

  ⚠️ 0.75, а не прежние 0.5. Причина считаная, не вкусовая: при 0.5 текст рисков
  давал контраст 2.85:1 при норме 4.5:1, а ST-21 п. 7.1.5 требует, чтобы
  обязательные сведения читались невооружённым глазом. При 0.75 - 5.53:1.
  Приглушать выборочно нельзя: opacity композитит всё поддерево целиком,
  вернуть ребёнку непрозрачность изнутри невозможно.
*/
.sg__stage {
  opacity: 0.75;
